...
namespace QuickVR.SampleInteraction
{
    ...
    public class TestInteractionUI : MonoBehaviour
    {
        ...
        {
            if (InputManagerKeyboard.GetKeyDown(UnityEngine.InputSystem.Key.Digit7))
            {
                _toggleDirectMove.isOn = true;
                _toggleWalkInPlace.isOn = false;
            }
            if (InputManagerKeyboard.GetKeyDown(UnityEngine.InputSystem.Key.Digit8))
            {
                _toggleDirectMove.isOn = false;
                _toggleWalkInPlace.isOn = true;
            }
            ...
            if (_show && _interactionManager)
            {
                ...
                _interactionManager.SetEnabledLocomotionSystem(QuickVRInteractionManager.DefaultLocomotionProvider.WalkInPlace, _toggleWalkInPlace.isOn);
